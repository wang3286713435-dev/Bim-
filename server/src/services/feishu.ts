...
function getDetailReliability(hotspot: FeishuHotspot): string {
  ...
  if (hotspot.tenderDetailSource?.includes('firecrawl-detail-json') || hotspot.tenderDetailSource?.includes('detail-enrichment+agent-firecrawl') || hotspot.tenderDetailSource?.includes('detail-enrichment+openclaw-browser')) return '深抓取详情';
  ...
  if (hotspot.tenderDetailSource?.includes('ceb-list+official-table')) return '官方列表可信';
